...
class Stage extends React.PureComponent {
  ...
  renderVenues(){
    ...
    return <Grid key={stage} item {...gridData}>
      ...
      <div>
          ...
          {Object.keys(times).map((time, i) => (
              <Grid key={i} container spacing={40}>
                      ...
                      { times[time] === "presentation" && <Grid key={stage} item {...gridData}>

                          {this.findPresentations({
                              presentation_venue : stage.toUpperCase() , presentation_time : time
                          })}
