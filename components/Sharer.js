...
const shareLink = (service, link, title = "", description = "") => {
  ...
  switch (service) {
    case 'linkedin':
      link = `https://www.linkedin.com/shareArticle?mini=true&url=${ _escape(link) }&title=${ _escape(title) }&summary=${ _escape(description) }`;
    break;
...
const Sharer = ({url, services, title, description}) => {
  ...
  return ( <div style={{marginTop: 20}}>{services.map(service => <TranslatedIconButton
        key={service}
        name={service}
